...
int main(void)
{
  ...
  // 初始化感測器
  MEMS_Write(LIS3DSH_CTRL_REG4_ADDR, 0x67);  // 100Hz + XYZ啟用
  MEMS_Write(LIS3DSH_CTRL_REG3_ADDR, 0x48);  // 中斷設定
  MEMS_Write(LIS3DSH_THRS1_1_ADDR,   0x40);  // 閾值（約1g）
  MEMS_Write(LIS3DSH_MASK1_B_ADDR,   0x3F);  // 軸遮罩B
  MEMS_Write(LIS3DSH_MASK1_A_ADDR,   0x3F);  // 軸遮罩A
  MEMS_Write(LIS3DSH_SETT1_ADDR,     0x01);  // 狀態機設定
  MEMS_Write(LIS3DSH_ST1_1_ADDR,     0x05);  // GNTH1
  MEMS_Write(LIS3DSH_ST1_2_ADDR,     0x11);  // CONT
  MEMS_Write(LIS3DSH_CTRL_REG1_ADDR, 0x01);  // SM1啟用
  ...
  // create task
  xTaskCreate(Handler_Task,   "Handler_Task", 128, NULL, 2, NULL);
...
/* USER CODE BEGIN MX_GPIO_Init_2 */
  ...
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);
  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, GPIO_PIN_SET);  // 預設拉高
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
  {
      if (GPIO_Pin == GPIO_PIN_0) {
          ...
          HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_14); // Red LED toggle
