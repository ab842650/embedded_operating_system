...
TaskHandle_t audioPlayerTaskHandle;
...
eButtonEvent getButtonEvent()
{
   ...
   uint32_t now = HAL_GetTick();
   bool current = buttonState();

   /* 1. Long press: triggered while holding */
   if (button_down && !long_press_fired && (now - button_down_ts > LONG_MILLIS_MIN)) {
       pending_event = LONG_PRESS;
       long_press_fired = true;
   }

   /* 2. Double press waiting */
   if (double_pending) {
       if (current && current != button_down) {
           pending_event = DOUBLE_PRESS;
           double_pending = false;
           long_press_fired = true;
       } else if (now - button_up_ts > DOUBLE_GAP_MILLIS_MAX) {
...
void AudioPlayerTask(void *pvParameters){
	...
	/* Retry SD mount until success (card may need time to power up) */
	{
		extern FATFS fs;
		FRESULT fr;
		xSemaphoreTake(fatfsMutex, portMAX_DELAY);
	...
	myprintf("Playback started at %lu ms\r\n", startTick);
	while (!IsFinished){
		/* Wait for DMA half/full transfer notification, timeout 5ms as safety net */
		...
		if (AudioState == AUDIO_STATE_STOP || AudioState == AUDIO_STATE_IDLE)
		{
			uint32_t elapsed = HAL_GetTick() - startTick;
			myprintf("Playback finished. Total time: %lu ms (%lu.%03lu sec)\r\n",
...
int main(void)
{
  ...
  MX_I2S3_Init();
  /* USER CODE BEGIN 2 */
  HAL_Delay(500);  /* Wait for SD card power-up */
  Mount_SD();
  ...
  xTaskCreate(AudioPlayerTask, "AudioPlayerTask", 512, NULL, 3, &audioPlayerTaskHandle);
  vTaskStartScheduler();
  /* USER CODE END 2 */
